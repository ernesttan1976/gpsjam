console.log("GPS Jam Layer: Background script loaded");
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    chrome.scripting.executeScript(
        ...
        func: () => {
          ...
          window.fetch = async function (...args) {
            ...
            try {
              ...
              if (
                url.includes("/db-api/spoofing_2p") ||
                url.includes("/db-api/jammed_agg")
              ) {
                ...
                try {
                  ...
                  window.dispatchEvent(
                    new CustomEvent("dataIntercepted", {
      ...
      (result) => {
        if (chrome.runtime.lastError) {
          console.error(
            "BACKGROUND: Main world injection failed:",
  ...
  if (message.type === "SPOOFING_DATA" || message.type === "JAMMING_DATA") {
    ...
    console.log(
      ...
      Array.isArray(message.data) ? message.data.length : typeof message.data
    );

    const storageKey =
      message.type === "SPOOFING_DATA" ? "spoofingData" : "jammingData";
    const timestampKey =
      message.type === "SPOOFING_DATA"
        ? "spoofingTimestamp"
        : "jammingTimestamp";
    const urlKey =
      message.type === "SPOOFING_DATA" ? "spoofingUrl" : "jammingUrl";
